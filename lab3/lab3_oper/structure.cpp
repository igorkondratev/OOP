...
Vector  Vector::operator +(const Vector &second)
{
    Vector V(0);
    for(int i = 0;i<getMaxSize();i++){
        V.arr[i]=(((int) (arr[i]))-48 && ((int) (second.arr[i]))-48)+'0';
    }
...
Vector & Vector::operator ++ (int){
    ...
    int i = size-1;
    while((int)(arr[i])!=48&&i!=0){
        i--;
    }
    if(i==size-1&& (int)(arr[i])==49){std::cout<<"Max value\n";}
    else {
        if (i == 0) { arr[size-1] = 1 + '0'; }
        else{
            arr[size-1] = 0 + '0';
            arr[i] = 1 + '0';
        }
